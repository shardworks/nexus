...
The Stacks exposes its runtime API via its `provides` object, accessed via `ctx.apparatus<StacksApi>('stacks')`.
...
| `nexus-clockworks/src/lib/db.ts` | Replace with `ctx.apparatus<StacksApi>('stacks')` calls |
| `nexus-sessions/src/lib/db.ts` | Replace with `ctx.apparatus<StacksApi>('stacks')` calls |
