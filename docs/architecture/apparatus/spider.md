...
- **Engine timeouts.** The `startedAt` field on engine instances is used by zombie detection (engines older than `zombieThresholdMs` with dead processes are reaped). A future extension may add a hard timeout that terminates still-alive engines that exceed a configured maximum runtime.
