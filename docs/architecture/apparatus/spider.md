...
> **Note (follow-up — out of scope for this commission).** The seven-state engine enum (with `'blocked'`), the six-state rig enum (with `'stuck'` and `'blocked'`) documented in [Rig Data Model](#rig-data-model), the `clockworks-retry`-driven `retryable` observability payload described below, and the `rig.stuck → writ open → stuck` arm in the [CDC handlers](#rig-terminalstuck-state--writ-transition) above will go stale once the planned pending+hold-metadata reshape lands. That reshape collapses `'blocked'` into `'pending'` carrying hold metadata and replaces the per-failure cascade with a single dispatch predicate. A dedicated refresh commission is required to rewrite this section, the CDC `rig.stuck` arm, and the lifecycle prose in the Rig Data Model against that new substrate; the present revision deliberately only flags the staleness rather than performing the rewrite.
