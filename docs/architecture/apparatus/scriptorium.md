...
> **Detached-HEAD reattach.** If an upstream caller detaches HEAD in the draft worktree before sealing begins (for example, a mid-flow `git checkout HEAD~1` to inspect the prior tree), the in-worktree rebase still produces correct commits but the bare clone's `refs/heads/<sourceBranch>` never advances — each retry iteration would resolve the stale pre-rebase SHA and the loop would exit with the misleading `Sealing failed after N retries` message. After every successful in-worktree rebase, the seal flow probes `git symbolic-ref HEAD` in the worktree; if that errors (HEAD is detached) the bare clone's source-branch ref is advanced via plain `git update-ref` to the worktree's rebased HEAD so the next iteration fast-forwards normally. The worktree's HEAD itself is intentionally left detached — only the bare clone's branch ref is repaired. The reattach surfaces operator-side as a single `[scriptorium]`-prefixed `console.warn` line naming the source branch and the old → new short SHAs.
