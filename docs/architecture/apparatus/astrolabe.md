...
mandate writ posted
  ...
  │       (provenance) and `depends-on` (precedence gate)
  ...
  │       own `depends-on` edge back to the originating mandate
  │     → the Spider's trySpawn gate uses each `depends-on` edge to hold
  ...
  │       and therefore carries no `depends-on` edge
...
- **Flat mode — single observation.** The engine calls `clerk.post({ type: 'mandate', title, body, codex, draft: true })` (no `parentId`) to create a single top-level draft `mandate` writ, then installs two outbound edges back to the originating mandate: `clerk.link(newWritId, planId, 'lifted from', 'astrolabe.lifted-from')` (provenance) and `clerk.link(newWritId, planId, 'depends on', 'depends-on')` (precedence gate).
- **Grouped mode — two or more observations.** The engine first creates a top-level draft container via `clerk.post({ type: 'observation-set', title: 'Observations from "{originating title}"', body, codex, draft: true })` whose body begins with a short preamble naming the originating writ and lists the child titles numerically, then installs a single outbound `astrolabe.lifted-from` edge from the container to the originating mandate. The container carries *no* `depends-on` edge — `observation-set` is non-dispatchable by type (it has no `rigTemplateMappings` entry), so Spider never attempts to pick it up, and a precedence edge on it would be dead data. For each observation record in record order, the engine then posts a draft `mandate` with `parentId` set to the container's id and installs a single outbound `depends-on` edge from the child back to the originating mandate. Children do not carry `astrolabe.lifted-from`; their provenance is discoverable via their `parentId` and the container's edge.

Every lifted `mandate` writ enters `new` (draft) phase, invisible to the Spider, ready for a curator (human or an overseer shipped separately) to promote to `open`. Once promoted, the `depends-on` edge enlists the Spider's `trySpawn` gate to hold the lifted writ until the originating mandate reaches a terminal state (release on `completed`/`cancelled`, cascade to `stuck` on `failed`) — the lifted observations describe concerns that presume the originating mandate has shipped, and the gate makes that latent dependency explicit in the writ graph rather than relying on curator memory. The engine is unconditional — empty, absent, or legacy-string observations are handled internally with a silent no-op, so no `when:` guard is needed. The two link kinds play complementary roles: `astrolabe.lifted-from` is the provenance trail (which observation came from which plan's planning run) and `depends-on` is the precedence gate (which lifted writ waits on which mandate). In grouped mode the two edges sit on different writs — the container carries `astrolabe.lifted-from` only, and each child carries `depends-on` only — but together they still form the full audit trail.
...
- **`astrolabe.plan-finalize`** — planning-phase terminator for the combined plan-and-ship rig. Validates that `plan.spec` is a non-empty string, parses the predicted-files cost signal from the spec's `<task-manifest>` (counting distinct path tokens across all `<task><files>` elements; 0 when no manifest is present or no path tokens are found), and atomically transitions `plan.status` from `'writing'` to `'completed'` while persisting `manifestFilesCount` on the PlanDoc in the same single patch. After the patch lands, when `manifestFilesCount` strictly exceeds the configured `astrolabe.predictedFilesThreshold` (default 15), the engine emits exactly one `astrolabe.plan.files-over-threshold` Clockworks event with payload `{ planId, count, threshold }`. The emission is best-effort: a Clockworks failure logs a `[astrolabe]` `console.warn` breadcrumb without failing the engine, and Clockworks itself is a soft dependency (declared in `recommends`) — when it isn't installed the count still lands on the PlanDoc and the engine succeeds silently. The framework never halts the pipeline on this signal; subscribers (sanctum-side instrumentation, future auto-decompose) decide what to do with it. Yields `{ spec }` so the downstream `implement` engine can read the spec via `${yields.plan-finalize.spec}` without posting a mandate writ.
- **`astrolabe.observation-lift`** — top-level writ fan-out stage. Reads the plan, validates its status is `'completed'` (placement after `plan-finalize` guarantees this), and walks `plan.observations`. If the field is not an array or is empty, the engine silently no-ops and yields `{ writIds: [] }`. Otherwise it produces top-level writs using one of two modes selected by a fixed threshold of two records. **Flat mode** (single observation): calls `clerk.post({ type: 'mandate', title, body, codex, draft: true })` (no `parentId`) and then installs two outbound edges back to the originating mandate — `clerk.link(newWritId, planId, 'lifted from', 'astrolabe.lifted-from')` (provenance) and `clerk.link(newWritId, planId, 'depends on', 'depends-on')` (precedence gate that hands the writ to the Spider's `trySpawn` gate once a curator promotes the draft). **Grouped mode** (two or more observations): first posts a top-level draft `observation-set` container whose title embeds the originating mandate's title (`Observations from "{title}"`) and whose body lists the observation titles with a short preamble naming the originating writ, then installs a single outbound `astrolabe.lifted-from` edge from the container to the originating mandate (the container carries *no* `depends-on` edge — `observation-set` is non-dispatchable and any precedence edge on it would be dead data); then, per observation record in record order, posts a draft `mandate` whose `parentId` is the container's id and installs exactly one outbound `depends-on` edge from that child back to the originating mandate (children do not carry `astrolabe.lifted-from`; their provenance is implied by the container's edge plus the parent-child relationship). The post and the link(s) are paired per record, so a mid-loop failure leaves a coherent prefix persisted. Fails fast on the first `clerk.post` or `clerk.link` error — already-created writs and links persist (invisible to the Spider until a curator publishes them), and a `clerk.link` failure leaves the in-flight writ posted but only partially linked for curator reconciliation. The engine never mutates the plan document; the two link kinds together — `astrolabe.lifted-from` for provenance, `depends-on` for precedence — form the audit trail. Yields `{ writIds }` — the ids of the draft observation writs in the same order as the observation records; the `observation-set` container id (when one exists) is *not* included in this array. Wired unconditionally in the plan-and-ship rig between `plan-finalize` and `seal`.
...
| `predictedFilesThreshold` | `number` | `15` | Soft-warn threshold for the predicted-files cost signal. The `astrolabe.plan-finalize` engine emits the `astrolabe.plan.files-over-threshold` Clockworks event when a plan's `manifestFilesCount` strictly exceeds this value. Must be a positive integer when supplied — the resolver fails loud on malformed input (wrong type, NaN, non-integer, zero, or negative). The default of 15 is a soft-warn level a few steps below the empirical 3.2× cost cliff at 20+ files-touched per implement-engine session. |
