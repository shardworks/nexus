...
mandate writ posted
  ...
  │     → walks plan.observations; per record, creates one draft `mandate`
  │       child writ under the originating mandate (parentId, draft: true),
  │       with title and body taken verbatim from each observation record
  │       and codex copied from the plan, then installs a `spider.follows`
  │       precedence-dependency edge from the lifted writ back to the
  │       originating mandate via clerk.link(newWritId, planId, 'depends on',
  │       'spider.follows') — the Spider's trySpawn gate uses this edge to
  │       hold each lifted writ until the originating mandate reaches a
  │       terminal state
...
The `observation-lift` engine runs immediately after `plan-finalize`. Placement is deliberate: the plan has reached its final `completed` state so the observation records are stable, while the originating `mandate` writ itself is still in `open` phase — Clerk rejects child-writ creation under a terminal parent, so this is the last safe window to attach children before the rig's seal stage transitions the mandate. The engine walks `plan.observations` and, per record, calls `clerk.post({ type: 'mandate', title, body, codex, parentId, draft: true })` to create the child writ and then `clerk.link(newWritId, planId, 'depends on', 'spider.follows')` to install a precedence-dependency edge from the lifted writ back to the originating mandate. Each created writ enters `new` (draft) phase, invisible to the Spider, ready for a curator (human or an overseer shipped separately) to promote to `open`. Once promoted, the `spider.follows` edge enlists the Spider's `trySpawn` gate to hold the lifted writ until the originating mandate reaches a terminal state (release on `completed`/`cancelled`, cascade to `stuck` on `failed`) — the lifted observations describe concerns that presume the originating mandate has shipped, and the gate makes that latent dependency explicit in the writ graph rather than relying on curator memory. The engine is unconditional — empty, absent, or legacy-string observations are handled internally with a silent no-op, so no `when:` guard is needed. The two edges play complementary roles: the parent-child edge from `clerk.post` is the provenance trail (which observation came from which plan), and the `spider.follows` edge from `clerk.link` is the precedence gate (which lifted writ waits on which mandate).
...
- **`astrolabe.observation-lift`** — child-writ fan-out stage. Reads the plan, validates its status is `'completed'` (placement after `plan-finalize` guarantees this), and walks `plan.observations`. If the field is not an array or is empty, the engine silently no-ops and yields `{ writIds: [] }`. Otherwise, for each `Observation` record it calls `clerk.post({ type: 'mandate', title: observation.title, body: observation.body, codex: plan.codex, parentId: planId, draft: true })` to create a draft child writ under the originating mandate, and then `clerk.link(newWritId, planId, 'depends on', 'spider.follows')` to install a precedence-dependency edge from the lifted writ back to the originating mandate — enlisting the Spider's `trySpawn` gate to hold each lifted writ until the originating mandate reaches a terminal state (release on `completed`/`cancelled`, cascade to `stuck` on `failed`). The post and the link are paired per record, so a mid-loop failure leaves a clean tail of fully-posted-and-linked writs and no half-state. Fails fast on the first `clerk.post` or `clerk.link` error — already-created post+link pairs persist under the mandate (invisible to the Spider until a curator publishes them), and a `clerk.link` failure leaves the current observation's writ posted but unlinked for curator reconciliation. The engine never mutates the plan document; the parent-child edge from `clerk.post` (provenance) and the `spider.follows` edge from `clerk.link` (precedence) together form the audit trail. Yields `{ writIds }` — the ids of the draft child writs in the same order as the observation records. Wired unconditionally in the plan-and-ship rig between `plan-finalize` and `seal`.
...
interface PlanDoc {
   ...
   * child `mandate` writ under the originating mandate so a curator
