...
- **Idempotency (load-bearing).** Handlers **must** be safe to invoke more than once for the same triggering event. Two replay shapes both produce duplicate invocations:
