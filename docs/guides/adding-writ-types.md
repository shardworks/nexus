...
> **Runtime wiring pending (requires T2/T3).** The structural shape (`WritTypeConfig`), its validator (`validateWritTypeConfig`), and their exports from `@shardworks/clerk-apparatus` ship today. The runtime that promotes declared per-type `allowedTransitions` into the transition validator and fires `childrenBehavior` triggers on child-terminal events lands in the T2/T3 refactor. You should declare full `WritTypeConfig` entries now — once T2/T3 merges, no migration is required.
...
> **Runtime wiring pending (requires T2/T3).** Today's Clerk hard-codes a mandate-flavored upward failure cascade only; the full `childrenBehavior` trigger firing lands in T2/T3. The rules the runtime will enforce: trigger firing (`allSuccess` on last-child-terminal with all-success; `anyFailure` on the first-failing child), idempotency (at most one parent transition per parent), short-circuit (`anyFailure` wins when both would match the same event), and `copyResolution` (copy the triggering child's `resolution` onto the parent when `true`).
