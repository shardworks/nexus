...
// ── crawl-continual — null idles silent ──────────────────────────────

describe('crawl-continual — null idles', () => {
  afterEach(() => clearGuild());

  it('records null idles silently (not in actions) while counting them', async () => {
    // null results don't get pushed to actions, but they do increment
    // idleCount so the loop auto-stops after maxIdleCycles.
    setGuild(makeGuild(async () => null));
