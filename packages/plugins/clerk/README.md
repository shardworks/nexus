...
- **Children-behavior cascade:** when any writ transitions to a terminal state, the children-behavior engine evaluates the parent's `WritTypeConfig.childrenBehavior` block and may lift the parent into its own terminal state. `anyFailure` is evaluated before `allSuccess` — a failing child wins precedence. `copyResolution: true` copies the triggering child's resolution onto the parent verbatim. Cascade is opt-in per type: a type with no `childrenBehavior` block is a silent no-op (parent and child lifecycles stay independent). Mandate opts in; piece and observation-set do not. There is no downward cascade — a terminal parent does not cancel non-terminal children. Cascade fires inside the transaction that triggered it (Phase 1); grandparent lift falls out naturally as the parent's own update event re-enters the watcher.
