...
The children-behavior engine — a Phase 1 watcher on the writs book — drives **both** cascade directions. When any writ transitions to a terminal state, the engine evaluates the relevant `WritTypeConfig.childrenBehavior` block(s) and applies the configured actions via `transition`:
...
- **Children-behavior cascade:** the children-behavior engine drives both directions. Upward (terminal child → parent lift) evaluates the parent's `childrenBehavior` (`anyFailure` before `allSuccess`; a failing child wins precedence; `copyResolution: true` copies the triggering child's resolution onto the parent verbatim). Downward (terminal parent → non-terminal-children cancellation) evaluates the triggering writ's own type's `parentTerminal` action when the writ reaches a `failure`- or `cancelled`-attr terminal — every non-terminal descendant is driven to the configured target with the configured resolution string. Cascade is opt-in per type: a type with no `childrenBehavior` block is a silent no-op. Mandate opts into all three triggers; piece and observation-set declare none. Cascade fires inside the transaction that triggered it (Phase 1); grandparent lift and grandchild cancel both fall out naturally as the next update event re-enters the watcher.
