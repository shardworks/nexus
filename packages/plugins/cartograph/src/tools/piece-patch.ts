...
/**
 * Patch a piece's companion doc. Per D6 the only mutable companion-doc
 ...
 * stage transitions go through `piece-transition`.
 */
export default tool({
  name: 'piece-patch',
  description: "Patch a piece's companion doc",
  instructions:
    "Updates a piece's companion doc. The only patchable field today is `codex`. " +
    'Title/body live on the writ row — edit them via `nsg writ edit`. Stage updates go ' +
    'through `piece-transition` so the writ phase and the companion stage move in one ' +
    'atomic step.',
  params: {
