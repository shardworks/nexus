...
/**
 * Show a vision by writ id. Composes the companion doc with the writ
 * row (D7) and returns either the lifecycle-aware text block (D18) or
 * the structured `{ ...doc, writ: { ... } }` JSON (D8).
 */
export default tool({
  name: 'vision-show',
  description: 'Show full detail for a vision',
  instructions:
    'Returns the vision companion doc joined with its underlying writ row, including ' +
