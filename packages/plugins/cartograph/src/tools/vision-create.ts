 ...
 * (D14 — no auto-transition; the patron runs `vision-transition` to
 * advance).
 */
export default tool({
  name: 'vision-create',
  ...
  instructions:
    ...
    "with `ext['cartograph'] = { stage: 'draft' }`. Use `vision-transition` to advance " +
    'the lifecycle. Title and body live on the writ row; edit them via `nsg writ edit`.',
  params: {
