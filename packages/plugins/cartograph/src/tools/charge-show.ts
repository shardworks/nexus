...
/**
 * Show a charge by writ id. Composes the companion doc with the writ
 * row (D7) and returns either the lifecycle-aware text block (D18) or
 * the structured `{ ...doc, writ: { ... } }` JSON (D8).
 */
export default tool({
  name: 'charge-show',
  description: 'Show full detail for a charge',
  instructions:
    'Returns the charge companion doc joined with its underlying writ row, including ' +
